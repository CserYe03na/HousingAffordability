...
```
This figure shows the long-run vacancy rate trends across New York City's five boroughs. Overall, vacancy rates remain consistently low throughout the entire period. Even though fluctuation exists, most boroughs keep them within a narrow range of 1% to 4%. The general trend suggests that NYC has a tight housing market and might experience persistent housing supply pressure for more than three decades.

To be specific, the Bronx shows a noticeable peak in the mid 1990s, reaching above 4%, then followed by a sharp decline in the early 2000s. This pattern may reflect earlier periods of disinvestment and later recovery. Manhattan shows the largest fluctuation in the later years, with a sharp spike in 2021 when vacancy rose to nearly 8%, followed by a rapid return to historically low levels by 2023. This unusual jump is likely related to short-term disruptions such as the COVID-19 pandemic rather than long-term structural changes in housing supply. Queens also experiences a temporary increase in vacancy around 2021, though to a much smaller extent, and quickly returns to very low levels. Staten Island remains the most stable borough throughout the period, with consistently low vacancy and minimal fluctuations. By 2023, vacancy rates across all five boroughs fall below 1%, highlighting an extremely tight post-pandemic housing market.
...
tm_shape(nyc_tract_vacancy, bbox = bbox_new) +
  tm_fill(
    "vacancy_rate",
    palette = "cividis",
    style = "jenks",
...
Manhattan shows the most noticeable cluster of higher-vacancy tracts, particularly around Midtown and parts of the Upper East Side and Lower Manhattan. These localized pockets stand out compared with the rest of the city. Queens also contains a few isolated tracts with comparatively elevated vacancy, appearing mainly near Long Island City and in some outer areas. In contrast, the Bronx, Brooklyn, and Staten Island remain almost entirely within the lowest vacancy categories, showing limited spatial variation.

Overall, even in a year with historically low citywide vacancy, certain neighborhoods, especially in Manhattan, display higher levels of unoccupied units, while most residential areas across NYC maintain very limited vacancy.
...
```
This bar chart shows the distribution of reported reasons why vacant housing units are not available for rent. The largest category is "multiple reasons reported", which means many vacant units face more than one constraint rather than a single cause at the same time.

Seasonal or occasional use is the second most common reason, which suggests that a significant portion of vacancies are associated with second homes, temporary occupancy, or infrequent usage. These units are technically vacant but are not intended to be part of the long-term rental supply. Units under renovation also represent a substantial share of unavailable housing, showing that short-term construction and improvement activities can temporarily reduce market supply. In contrast, relatively few units are reported as being intentionally held vacant. Generally, this distribution reveals that most vacancy in the market is driven by practical factors rather than by holding of housing from renters.
...
fviz_pca_biplot(
  ...
  ggtitle("Structural Housing Characteristics Associated with Vacancy, 2023") +
  theme_minimal() 
```
The biplot describes the main structural differences among vacant housing units using two principal components.

The first dimension mainly reflects overall building scale and vertical form. On the right side of the x-axis, variables such as log units, year built, number of stories, and unit floor level load positively. We can conclude that units on the right side tend to be in larger, taller, and newer buildings, and are located on higher floors. In contrast, on the left side of the x-axis, elevator and condo/co-op load in the opposite direction, suggesting a different building type structure.

The second dimension is mainly driven by interior unit features, including number of rooms, number of bedrooms, and number of bathrooms. Units with higher values on PC2 tend to have larger interior space and more amenities, while units with lower PC2 values are more compact and have fewer interior features.

The variable log_lastrent has a very short loading vector. That proves the past rent level contribute minimally to the structural variation captured by the first two principal components and do not strongly distinguish among vacant units.

To sum up, this biplot shows that vacant housing units in NYC differ mainly along two structural dimensions:
...
```
The mosaic pairs plot shows a clear pattern: higher-income households are much more likely to appear in the high-rent categories, while lower-income households cluster heavily in the cheapest rent brackets. Rent levels in NYC closely track economic capacity-households do not spread randomly across rent tiers but sort themselves according to what they can realistically afford.

An important insight is that while demographic factors like age, race, or household type show some small differences, none of them explain rent variation as strongly as income does. Income overwhelmingly determines which rent levels households end up in, reinforcing the idea that NYC's rental market is structured primarily by affordability rather than demographic differences.
...
ggplot(plot_df, aes(x = tenure, y = pct, fill = cost_burden)) +
  ...
  labs(
    ...
    subtitle = "Owners with mortgages vs Renters with rent",
    x = "",
...
```
This stacked bar chart compares housing cost burden between homeowners with mortgages and renters in 2023. A clear difference appears between the two groups. Among homeowners with mortgages, about one third of households fall into the "not burdened" category. That is, they spend less than 30% of their income on housing. Only a relatively small share of mortgage holders are moderately or severely burdened.
...
plot_df2 <- df23 |>
  ...
  group_by(income_bracket, rent_burden) |>   
  summarise(wt = sum(fw, na.rm = TRUE), .groups = "drop_last") |> 
  group_by(income_bracket) |>               
  mutate(pct = wt / sum(wt)) |>             
  ungroup() |>
...
```{r}
# Plot4.2: housing burden vs. household income for renter
#| fig-width: 18
#| fig-height: 12
ggplot(plot_df2,
...
```{r}
# Plot4.3: housing burden vs. household income for owner
#| fig-width: 18
#| fig-height: 12
...
```{r}
# Plot4.4: Housing affordability vs vacancy scatter
cividis_pal <- c(
  "black",
  "steelblue",
  "darkred",  
...
Different boroughs occupy somewhat distinct regions of the plot, with Staten Island generally appearing more affordable and Manhattan less affordable, but vacancy levels remain low across boroughs in most years.
